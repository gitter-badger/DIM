...
(function () {
  ...
  angular.module('dimApp')
    .controller('dimPlatformChoiceCtrl', PlatformChoiceCtrl);

  PlatformChoiceCtrl.$inject = ['$scope', '$rootScope', 'dimBungieService', 'dimConfig', 'dimPlatformIds'];

  function PlatformChoiceCtrl($scope, $rootScope, dimBungieService, dimConfig, dimPlatformIds) {
    var vm = this;
    ...
    vm.update = function update() {
      ...
      $rootScope.$broadcast('dim-active-platform-updated', vm.active);
    };

    activate();

    function activate() {
      dimBungieService.getPlatformIds()
        .then(function(platforms) {
          var xbl = (!_.isEmpty(platforms.xbl) ? platforms.xbl : null);
          var psn = (!_.isEmpty(platforms.psn) ? platforms.psn : null);
          ...
          vm.platforms = _.values(dimPlatformIds);
          vm.active = dimConfig.active = (!_.isEmpty(dimPlatformIds.xbl) ? dimPlatformIds.xbl : dimPlatformIds.psn);

          // TODO Get previous setting from local storage.

          $rootScope.$broadcast('dim-platforms-updated', dimPlatformIds);
          $rootScope.$broadcast('dim-active-platform-updated', vm.active);
        });
    }
